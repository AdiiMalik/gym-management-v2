...
import React from 'react';
import { NavLink } from 'react-router-dom';
import { Home, Users, Settings as SettingsIcon, X, Shield, Package, ShoppingCart } from 'lucide-react';
  ...
  return (
    <>
      ...
      >
        ...
        {/* Nav Links */}
        <nav className="flex-1 flex flex-col py-4 space-y-1">
          <NavLink to="/dashboard" className={navItemClasses} onClick={() => setSidebarOpen(false)}>
            <Home className="mr-3 w-5 h-5" />
            <span>Dashboard</span>
          </NavLink>

          <NavLink to="/members" className={navItemClasses} onClick={() => setSidebarOpen(false)}>
            <Users className="mr-3 w-5 h-5" />
            <span>Members</span>
          </NavLink>

          {/* Products Page */}
          <NavLink to="/products" className={navItemClasses} onClick={() => setSidebarOpen(false)}>
            <Package className="mr-3 w-5 h-5" />
            <span>Products</span>
          </NavLink>

          {/* POS / Sales */}
          <NavLink to="/pos" className={navItemClasses} onClick={() => setSidebarOpen(false)}>
            <ShoppingCart className="mr-3 w-5 h-5" />
            <span>POS / Sales</span>
          </NavLink>

          <NavLink to="/settings" className={navItemClasses} onClick={() => setSidebarOpen(false)}>
            <SettingsIcon className="mr-3 w-5 h-5" />
            <span>Settings</span>
          </NavLink>
<NavLink
  ...
  onClick={() => setSidebarOpen(false)}
>
  <Box className="mr-3 w-5 h-5" />
  <span>Manage Products</span>
</NavLink>
          {/* Super Admin Only */}
          {user?.role === 'superadmin' && (
            <NavLink to="/superadmin" className={navItemClasses} onClick={() => setSidebarOpen(false)}>
              <Shield className="mr-3 w-5 h-5" />
              <span>Super Admin Panel</span>
            </NavLink>
          )}
